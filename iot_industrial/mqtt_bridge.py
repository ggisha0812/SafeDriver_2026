...
# AQUI ESTA LA CORRECCION: Cambiado "backend" por "localhost"
...
def evaluar_alerta(bpm, parpadeos, velocidad):
    ...
    elif velocidad > UMBRAL_VELOCIDAD:
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        bpm          = payload.get("valor_bpm", 75)
        parpadeos    = payload.get("frecuencia_parpadeo", 15)
        velocidad    = payload["velocidad_kmh"]
        ahora        = time.time()

        nivel, tipo = evaluar_alerta(bpm, parpadeos, velocidad)
        ...
        if vehiculo_id not in cache_local:
            ...
            elif variacion_significativa(ultimo["bpm"], bpm) or variacion_significativa(ultimo["parpadeos"], parpadeos) or variacion_significativa(ultimo["velocidad"], velocidad):
                guardar_en_db = True
                razon_filtro = "Variación > 5%"

        cache_local[vehiculo_id] = {"bpm": bpm, "parpadeos": parpadeos, "velocidad": velocidad, "tiempo": ahora}

        print(f"[RX Vehiculo {vehiculo_id}] BPM: {bpm} | Parpadeos: {parpadeos} | Vel: {velocidad}")
        ...
        if guardar_en_db:
            data = {
                ...
                "tipo": tipo,
                "valor_bpm": bpm,
                "parpadeos_por_minuto": parpadeos,
